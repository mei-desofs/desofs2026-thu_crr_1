  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        // Backend resolves carrier from JWT — no id in request (V8.2.2)
        const data = await apiGet<Order[]>("/orders/carrier");
        setOrders(data ?? []);
      } catch {
  ...
  return (
    <div className="py-8">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold text-white mb-8">My Deliveries</h1>
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-slate-300 mt-4">Loading deliveries…</p>
          </div>
        ) : orders.length === 0 ? (
          <div className="text-center py-12">
          ...
          <div className="space-y-4">
            {orders.map((order, index) => (
              <DeliveryCard
                key={order.orderId}
                order={order}
                deliveryNumber={orders.length - index}
              />
            ))}
          </div>
        )}
    ...
    <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-4">
          <span className="text-white font-semibold">Delivery #{deliveryNumber}</span>
